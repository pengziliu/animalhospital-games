...
const icons: LucideIcon[] = [BookOpen, Code2, Flame, Users, Medal, Boxes, Trophy, Zap];
...
export default function HomePageClient({ home, locale, articles, recentArticles }: { home: Home; locale: string; articles: ContentItem[]; recentArticles: ContentItem[] }) {
  const YOUTUBE_VIDEO_ID = "vBrjJ6alV84";
  ...
  return (
    <div className="space-y-16">
      ...
      {/* Hero Section — with hero.webp background image */}
      <section className="relative -mt-16 -mx-4 overflow-hidden sm:-mx-6 lg:-mx-8">
        <div
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-8 text-center sm:px-6 sm:py-12 lg:px-8 lg:py-16">
          <span className="inline-flex items-center rounded-md border border-[hsl(var(--nav-theme-light))] bg-[hsl(var(--nav-theme))] px-2.5 py-0.5 text-xs font-semibold text-primary-foreground">{home.hero.eyebrow}</span>
          <h1 className="mt-3 text-3xl font-extrabold tracking-tight text-white drop-shadow-md sm:mt-4 sm:text-5xl lg:text-6xl">{home.hero.title}</h1>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-relaxed text-white/80 sm:hidden">{home.hero.description.length > 80 ? home.hero.description.slice(0, 80) + "…" : home.hero.description}</p>
          <p className="mx-auto mt-4 hidden max-w-3xl rounded-2xl bg-black/50 px-6 py-4 text-base leading-relaxed text-white/90 backdrop-blur-sm sm:block">{home.hero.description}</p>
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-1.5">
              {home.hero.stats.map((stat, i) => (
                <span key={i} className={`inline-flex items-center gap-1 rounded-full bg-white/10 px-2.5 py-1 text-xs font-medium text-white/90 ring-1 ring-white/20 backdrop-blur-sm ${i >= 2 ? 'hidden sm:inline-flex' : ''}`}>{stat}</span>
              ))}
            </div>
          </div>
        </div>
